...
def main():
    ...
    ap.add_argument(
        "--source",
        required=True,
        help="Folder of campaign folders (e.g. data/raw_dedup)",
    )
    ap.add_argument(
        "--campaign-cmd", required=True, help="Command template (use {campaign_dir})"
    )
    ap.add_argument(
        "--max-campaigns", type=int, default=0, help="Limit campaigns per run"
    )
    args = ap.parse_args()
