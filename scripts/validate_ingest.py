...
def validate(df: pd.DataFrame, root: Path) -> dict:
    ...
    for _, r in df.iterrows():
        if pd.isna(r.get("doc_type")) or str(r.get("doc_type")).strip() == "":
            ...
            issues["unknown_doc_type"].append(
                {"path": r.get("rel_path"), "value": r["doc_type"]}
            )
    ...
    for pth, cnt in dup_paths.items():
        if cnt > 1:
            issues["duplicate_path"].append({"path": pth, "count": int(cnt)})
    ...
    for h, cnt in dup_hash.items():
        if h and cnt > 1:
            ...
            issues["duplicate_content"].append(
                {"hash": h, "count": int(cnt), "paths": paths}
            )
    ...
    if "n_chunks" in df.columns:
        for _, r in df.iterrows():
            ...
            if n == 0:
                issues["empty_or_short"].append(r.get("rel_path"))
            if n > 300:
                issues["chunk_anomaly"].append(
                    {"path": r.get("rel_path"), "n_chunks": n}
                )
...
def main():
    ...
    ap.add_argument(
        "--root",
        required=True,
        help="Root of extracted campaign (e.g., data/raw/211_LA)",
    )
