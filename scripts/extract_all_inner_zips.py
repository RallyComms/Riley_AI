import argparse, os, shutil, zipfile, fnmatch
...
def flatten_inner_zip(inner_zip: Path, extracted_tmp_root: Path, raw_out: Path, dry_run: bool):
    # 1) extract inner zip
...
def main():
    ap = argparse.ArgumentParser(description="Flatten inner zips into data/raw/<Campaign> folders")
    ap.add_argument("--mega", help="Path to the mega zip that contains the 18 inner zips (optional)")
    ap.add_argument("--inner-zips", default=str(INNER_ZIPS), help="Folder containing the 18 inner zip files")
    ap.add_argument("--out", default=str(RAW_OUT), help="Destination for flat campaign folders (data/raw)")
    ap.add_argument("--tmp", default=str(EXTRACTED), help="Temp extraction root (short path recommended)")
    ap.add_argument("--dry-run", action="store_true", help="Plan only; do not copy campaigns")
    ap.add_argument("--delete-tmp", action="store_true", help="Delete temp extracted trees when done")
    args = ap.parse_args()
