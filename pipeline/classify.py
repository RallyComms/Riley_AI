...
def best_rule_label(text, taxonomy, patterns, filename=""):
    ...
    for label, syns in taxonomy.get("synonyms", {}).items():
        hits = sum(1 for s in syns if s.lower() in text_low) + sum(1 for s in syns if s.lower() in name_low)
        if hits >= 2: scores[label] = scores.get(label, 0) + 0.3
        elif hits == 1: scores[label] = scores.get(label, 0) + 0.15
    if not scores:
