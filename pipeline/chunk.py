...
def main(campaign_root: str, cfg_path="configs/chunking.yaml"):
    ...
    tgt = cfg["defaults"]["target_tokens"]; ov = cfg["defaults"]["overlap_tokens"]
    ...
    with open(extracted, encoding="utf-8") as fin, open(out_chunks,"w",encoding="utf-8") as fout:
        for line in fin:
            r = json.loads(line); ekind = r.get("extract",{}).get("type")
            sp = r["source_path"]
            mrow = meta[meta["source_path"]==sp]
            if mrow.empty:
                doc_type="messaging_one_pager"; index_mode="full"; pii_flags="None"
            else:
